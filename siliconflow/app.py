...
def transcribe(audio_file: bytes, output: str = "txt"):
    output=service.audio_2_text(audio_file)
...
def greet(question, audio_input, correct_answer, role, tips: str, ):
    ...
    logger.info(f"greet: {question}, {audio_input}, {correct_answer}, {role}, {tips}")
    # 保存音频文件并获取路径
    ...
    sr, audio_data = service.text_2_audio(content) # 调用服务的 text_2_audio 方法，将 content 作为参数传入，返回一个字符串，赋值给 audio_output 变量，作为函数的返回值。
    
    return [f"{audio_ouput}",f"{content}",(sr, audio_data)]
...
demo = gr.Interface(fn=greet,
    ...
    outputs=[gr.Textbox(label="面试者回答文本"),gr.Textbox(label="AI评估",lines=10,max_lines=20),gr.Audio(label="AI评估语音")])
